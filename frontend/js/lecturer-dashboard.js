/* ========================================================
   Lecturer Dashboard — MyEduConnect
======================================================== */
...
/* ── Stats ── */
function loadStats() {
    fetch('/lecturer/stats', { credentials: 'include' })
        .then(r => r.ok ? r.json() : null)
        .then(d => {
            if (!d) return;
            document.getElementById('statCourses').textContent     = d.courses     ?? '—';
            document.getElementById('statStudents').textContent    = d.students    ?? '—';
            document.getElementById('statAssignments').textContent = d.assignments ?? '—';
            document.getElementById('statSubmissions').textContent = d.submissions ?? '—';
        });
}
...
function renderOverviewCourses(courses) {
    ...
    if (!courses.length) {
        tbody.innerHTML = '<tr><td colspan="4" class="loading-row">No courses assigned to your account.<br><small>Ensure your user email matches the lecturers table.</small></td></tr>';
        return;
    }
...
function loadAllAssignments(courses) {
    ...
    Promise.all(courses.map(c =>
        ...
        results.forEach(r => { myAssignments[r.courseId] = r.list; });
...
function renderAssignmentsTable(filterCourseId) {
    ...
    myCourses.forEach(c => {
        ...
        list.forEach(a => {
            rows.push({ ...a, courseName: `${c.course_code} — ${c.course_name}` });
        });
    });
    if (!rows.length) {
...
function setupAssignmentForm() {
    ...
    form.addEventListener('submit', e => {
        ...
        .then(r => r.json())
        .then(d => {
            if (d.message === 'Assignment created') {
...
function setupMaterials() {
    ...
    form.addEventListener('submit', async e => {
        ...
        if (d.message === 'Material uploaded') {
...
function deleteMaterial(id) {
    ...
    fetch(`/materials/${id}`, { method: 'DELETE', credentials: 'include' })
        .then(r => r.json())
        .then(d => {
            showToast(d.message === 'Deleted' ? 'Material deleted' : d.message,
...
function renderStudentsTable(students) {
    ...
    if (!students.length) {
        tbody.innerHTML = '<tr><td colspan="4" class="loading-row">No students enrolled in this course.</td></tr>';
        return;
    }
...
function renderSubmissionsTable(subs) {
    ...
    tbody.innerHTML = subs.map((s, i) => {
        ...
        <tr id="sub-row-${s.id}">
            ...
            <td style="color:var(--muted)">${fmtDate(s.submitted_at)}</td>
...
function gradeSubmission(id) {
    ...
    .then(r => r.json())
    .then(d => {
        showToast(d.message === 'Graded' ? 'Grade saved' : d.message,
                  d.message === 'Graded' ? 'success' : 'error');
        if (d.message === 'Graded') {
